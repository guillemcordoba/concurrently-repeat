...
import defaults from "concurrently/src/defaults.js";
import yargs from "yargs";

const args = yargs(process.argv)
  .usage("$0 [options] <command> <number of repeats>")
  .help("h")
  .alias("h", "help")
  .version("v", "0.0.1")
  .alias("v", "V")
  ...
  .group(
    ["m", "n", "name-separator", "raw", "s", "no-color", "hide"],
    "General"
  )
  .group(["p", "c", "l", "t"], "Prefix styling")
  .group(["i", "default-input-target"], "Input handling")
  .group(["k", "kill-others-on-fail"], "Killing other processes")
  .group(["restart-tries", "restart-after"], "Restarting").argv;

const names = (args.names || "").split(args.nameSeparator);
const command = getCommand();
const commands = Array.from({ length: repeatTimes() }).map((_, index) => ({
  command,
  name: names[index],
}));
...
concurrently(commands, {
  ...
  raw: args.raw,
  hide: args.hide ? args.hide.split(",") : [],
  prefix: args.prefix,
...
function repeatTimes() {
  return parseInt(process.argv[process.argv.length - 1]);
}
